...
function handleAccept(id, variantNum, lines, targetFile, paramValues) {
  ...
  if (cssContent) {
    replacement.push(indent + commentSyntax.open + ' uizze-carbonize-start ' + id + ' ' + commentSyntax.close);
    ...
    if (paramValues && Object.keys(paramValues).length > 0) {
      ...
      replacement.push(indent + commentSyntax.open + ' uizze-param-values ' + id + ': ' + JSON.stringify(paramValues) + ' ' + commentSyntax.close);
    }
    replacement.push(indent + commentSyntax.open + ' uizze-carbonize-end ' + id + ' ' + commentSyntax.close);
  }
