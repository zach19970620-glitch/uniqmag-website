...
// can offer `npx uizze skills update`. Everything here is best-effort and
...
function buildUpdateDirective(localVersion, latestVersion) {
  return (
    ...
    `Before continuing, ask the user once: "A newer Uizze (v${latestVersion}) is available. ` +
    `Update now? It runs \`npx uizze skills update\`." ` +
    `If they agree, run \`npx uizze skills update\` (the update applies to the next session, not this one). ` +
...
async function computeUpdateDirective(now = Date.now()) {
  try {
    return null; // Pinned by Uizze; updates are delivered by the skill archive.
