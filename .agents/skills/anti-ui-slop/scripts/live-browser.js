...
(function () {
  ...
  function buildConfigureRow() {
    ...
    input.type = 'text';
    input.placeholder = selectedAction === 'uizze' ? 'describe what you want…' : 'refine further (optional)…';
    ...
    if (!document.getElementById(PREFIX + '-configure-input-style')) {
      ...
      s.textContent =
        ...
        '#' + PREFIX + '-configure-voice[data-listening="true"] svg { animation: uizze-configure-voice-pulse 1.1s ease-in-out infinite; }' +
    ...
    evtSource.onmessage = (e) => {
      ...
      switch (msg.type) {
        case 'connected':
          ...
          if (!hasProjectContext) showToast('No PRODUCT.md found. Variants will be brand-agnostic. Run /uizze init to generate one.', 7000);
...
void main() {
  ...
  function syncAgentPollingUi(connected) {
    ...
    globalBarBrandEl.setAttribute('aria-label', connected
      ? 'Uizze live mode'
      : 'Uizze live mode — agent not polling');
