 ...
 *
 * Usage:
 *   npx uizze poll                         # Block until browser event, print JSON
 *   npx uizze poll --stream                # Experimental: keep polling; one JSON line per event
 *   npx uizze poll --timeout=600000        # Custom timeout (ms); default is long-poll friendly
 *   npx uizze poll --reply <id> done       # Reply "done" to event <id>
 *   npx uizze poll --reply <id> error "msg" # Reply with error
 */
...
function readServerInfo() {
  ...
  if (!record) {
    console.error('No running live server found. Start one with: npx uizze live');
    process.exit(1);
  }
  return record.info;
}
...
function validateReplyArgs({ id, status }) {
  const usage = "Usage: npx uizze poll --reply <id> <status> [--file path] [--data '<json>'] [message]";
  if (!id || id.startsWith('--')) {
...
function handlePollError(err) {
  if (err.code === 'AUTH_FAILED') {
    ...
    console.error('Try restarting: npx uizze live stop && npx uizze live');
    process.exit(1);
  }
  if (err.cause?.code === 'ECONNREFUSED') {
    console.error('Live server not running. Start one with: npx uizze live');
    process.exit(1);
  }
...
Harness note:
  ...
  // Reply mode: npx uizze poll --reply <id> <status> [--file path] [--data '<json>'] [message]
