...
│
├── 账号
│   ├── /login            登录（验证码 / 密码 / 微信；密码 Tab 可弹窗重置密码）
│   ├── /register         密码注册
...
- 密码 Tab 右下「忘记密码？」打开弹窗；接口若返回「请使用验证码登录或先设置密码」，「先设置密码」打开同一弹窗。
- 重置/首次设置弹窗：手机号（可预填登录框）、验证码 + 倒计时、密码、确认密码、显隐。成功后直接登录。
...
- 密码 < 6 位 →「密码至少 6 位」
- 接口错误原文展示；若为「请使用验证码登录或先设置密码」，「先设置密码」可点，打开重置弹窗

**重置密码弹窗**（无独立路由）

- 标题「重置密码」
- 手机号（预填登录页已输入的号）、验证码 +「获取验证码」、密码、确认密码
- 主按钮「确认并登录」；成功后按登录成功跳转
- 从未设过密码的账号也走这一套（不是另一条产品流程）
...
- 登录密码：未设置 / 已设置；点「设置」或「修改」弹窗（未绑手机则去绑定）

**设置密码弹窗**：说明验证码发到当前绑定号（展示脱敏号）；验证码、密码、确认密码。主按钮「确认设置」。

**修改密码弹窗**：当前密码、新密码、确认密码。主按钮「确认修改」。新旧不能相同。忘了当前密码须退出后走登录页重置。
